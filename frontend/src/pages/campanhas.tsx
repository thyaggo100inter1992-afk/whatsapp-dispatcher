...
export default function Campanhas() {
  ...
  const [managingAccountsCampaignId, setManagingAccountsCampaignId] = useState<number | null>(null);
  ...
  const handleDownloadReport = async (campaignId: number, campaignName: string) => {
    try {
      setDownloadingReport(campaignId);
      toast.info('📊 Gerando relatório Excel...');

      const response = await campaignsAPI.downloadReport(campaignId);
      ...
      link.href = url;
      link.download = `Relatorio_${campaignName.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.xlsx`;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-dark-900 via-dark-800 to-dark-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-2xl text-white/70">Carregando campanhas...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-xl text-white/80 font-medium">
                    Gerencie suas campanhas de envio em massa via API Oficial
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-4 mb-3 flex-wrap">
                      ...
                      {getStatusBadge(campaign.realStatus || campaign.status)}
                    </div>
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {(campaign.status === 'running' || campaign.status === 'paused') && (
                      <button
                        onClick={() => setManagingAccountsCampaignId(campaign.id)}
      ...
      {/* Campaign Accounts Manager Modal */}
      {managingAccountsCampaignId && (
        <CampaignAccountsManager
          campaignId={managingAccountsCampaignId}
          onClose={() => {
            setManagingAccountsCampaignId(null);
            loadCampaigns();
          }}
          toast={toast}
        />
