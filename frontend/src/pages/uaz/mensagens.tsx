...
import { FaEnvelope, FaCheckCircle, FaTimesCircle, FaClock, FaFilter, FaSearch, FaMobileAlt, FaArrowLeft } from 'react-icons/fa';
...
export default function MensagensUazPage() {
  ...
  const loadMessages = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setTotalMessages(response.data.total || response.data.data?.length || 0);
      } else {
  ...
  const filteredMessages = messages.filter((message) => {
    ...
    const matchesSearch = 
      ...
      (message.instance_name && message.instance_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (message.contact_name && message.contact_name.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    const matchesInstance = 
      instanceFilter === 'all' || 
      message.instance_id.toString() === instanceFilter;
  ...
  const uniqueInstances = Array.from(
    new Set(messages.map(m => JSON.stringify({ id: m.instance_id, name: m.instance_name || `Instância ${m.instance_id}` })))
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-5xl font-black text-white">
                  Histórico de Mensagens QR Connect
                </h1>
                <p className="text-white/60 text-lg mt-1">
                  Todas as mensagens enviadas via WhatsApp QR Connect (Baileys)
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="md:col-span-2 bg-white/5 border-2 border-white/10 rounded-xl p-4">
                ...
                <input
                  ...
                  placeholder="Número, tipo de mensagem ou instância..."
              ...
              <div className="bg-white/5 border-2 border-white/10 rounded-xl p-4">
                ...
                <select
                  ...
                  className="w-full px-6 py-4 bg-dark-700 text-white text-base rounded-xl border-2 border-cyan-500/30 focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/30 transition-all cursor-pointer"
                >
                  <option value="all" className="bg-dark-700">📱 Todas</option>
                  {uniqueInstances.map((inst) => (
                    <option key={inst.id} value={inst.id} className="bg-dark-700">
                      📱 {inst.name}
                    </option>
                  ))}
                </select>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-gradient-to-br from-green-500/20 to-green-600/20 border-2 border-green-500/40 p-8 rounded-xl hover:scale-105 transition-transform shadow-lg shadow-green-500/20">
              <div className="text-green-300 text-lg font-bold mb-3 flex items-center gap-2">
                <span className="text-3xl">✅</span> Entregues
              </div>
              <div className="text-5xl font-black text-white">
                {messages.filter(m => m.status === 'delivered').length}
              </div>
              <div className="text-green-200/60 text-sm mt-2">com sucesso</div>
            </div>
            <div className="bg-gradient-to-br from-purple-500/20 to-purple-600/20 border-2 border-purple-500/40 p-8 rounded-xl hover:scale-105 transition-transform shadow-lg shadow-purple-500/20">
              <div className="text-purple-300 text-lg font-bold mb-3 flex items-center gap-2">
                <span className="text-3xl">👀</span> Lidas
              </div>
              <div className="text-5xl font-black text-white">
                {messages.filter(m => m.status === 'read').length}
              </div>
              <div className="text-purple-200/60 text-sm mt-2">visualizadas</div>
            </div>
          ...
          <div className="bg-gradient-to-br from-white/10 to-white/5 border-2 border-white/20 rounded-xl overflow-hidden shadow-lg">
            {loading ? (
              <div className="text-center p-20">
                <div className="animate-spin rounded-full h-20 w-20 border-b-4 border-cyan-500 mx-auto mb-6"></div>
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-cyan-600/30 to-blue-600/30 border-b-2 border-white/20">
                    ...
                    {filteredMessages.length === 0 ? (
                      ...
                      filteredMessages.map((message) => (
                        <tr key={message.id} className="border-b border-white/10 hover:bg-white/5 transition-all">
                          ...
                          <td className="p-5 text-white/90 font-medium">{getTypeLabel(message.message_type)}</td>
                          <td className="p-5">
                            <span className="text-cyan-300 font-bold bg-cyan-500/10 px-3 py-1 rounded-lg">
                              {message.instance_name || `Instância ${message.instance_id}`}
                            </span>
                          </td>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between p-8 border-t-2 border-white/20 bg-white/5">
                  <div className="text-white font-bold text-base">
                    📄 Página <span className="text-cyan-400">{page}</span> de <span className="text-cyan-400">{totalPages}</span> • 
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      disabled={page === 1}
                      className="px-6 py-3 bg-gradient-to-r from-cyan-500/20 to-cyan-600/10 hover:from-cyan-500/30 hover:to-cyan-600/20 border-2 border-cyan-500/50 text-cyan-200 font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed text-base"
                    >
                      ⬅️ Anterior
                    </button>
                    <button
