...
import {
  FaServer, FaTrash, FaSync, FaDownload
} from 'react-icons/fa';
...
export default function AdminLogsBackend() {
  ...
  useEffect(() => {
    // Auto-scroll para o final quando novos logs chegam (apenas se autoScroll estiver ativo)
  ...
  const loadLogs = async () => {
    try {
      const response = await api.get('/admin/system-logs/backend?limit=1000');
  ...
  const handleClearLogs = async () => {
    if (!confirm('Tem certeza que deseja limpar todos os logs do backend?')) return;
    
    try {
      await api.delete('/admin/system-logs/backend');
      setLogs([]);
  ...
  if (error) {
    return (
      <AdminLayout
        ...
        subtitle="Console do servidor em tempo real"
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 text-white cursor-pointer">
                ...
                />
                <span className="font-medium">Auto-Refresh (2s)</span>
              </label>
        ...
        <div className="bg-black/90 backdrop-blur-sm rounded-xl shadow-2xl border-2 border-purple-500/30 p-6 font-mono text-sm overflow-hidden">
          ...
          >
            {loading && logs.length === 0 ? (
              <div className="text-gray-400 text-center py-8">Carregando logs...</div>
            ) : logs.length === 0 ? (
