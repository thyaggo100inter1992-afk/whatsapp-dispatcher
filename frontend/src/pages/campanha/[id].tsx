...
export default function CampanhaDetalhes() {
  ...
  const loadMessages = async () => {
    try {
      const response = await campaignsAPI.getMessages(Number(id));
      if (response.data.success) {
  ...
  const loadActivityLog = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'}/campaigns/${id}/activity-log`, {
        headers: {
  ...
  const loadButtonsStats = async () => {
    try {
      const token = localStorage.getItem('@WhatsAppDispatcher:token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'}/campaigns/${id}/buttons-stats`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const loadAllContacts = async () => {
    ...
    try {
      const token = localStorage.getItem('@WhatsAppDispatcher:token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'}/campaigns/${id}/contacts`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      ...
      if (data.success) {
        ...
        toast.error('Erro ao carregar contatos: ' + data.error);
      }
    } catch (error) {
  ...
  const handlePause = async () => {
    ...
    try {
      const response = await campaignsAPI.pause(Number(id));
      if (response.data.success) {
  ...
  const handleResume = async () => {
    ...
    try {
      const response = await campaignsAPI.resume(Number(id));
      if (response.data.success) {
  ...
  const handleCancel = async () => {
    ...
    try {
      const response = await campaignsAPI.cancel(Number(id));
      if (response.data.success) {
  ...
  const handleReactivateAccount = async (accountId: number, accountName: string) => {
    const confirmed = await confirm({
      title: 'Reativar Conta',
      message: `Deseja reativar manualmente a conta ${accountName}?\n\nEla voltará a enviar mensagens nesta campanha.`,
      type: 'info',
    ...
    try {
      const token = localStorage.getItem('@WhatsAppDispatcher:token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'}/campaigns/${id}/add-account`, {
        method: 'POST',
        headers: {
      ...
      const data = await response.json();
      if (data.success) {
        toast.success(`✅ Conta ${accountName} reativada com sucesso!`);
        loadCampaign();
        loadActivityLog();
      } else {
        toast.error('Erro ao reativar conta: ' + data.error);
      }
    } catch (error: any) {
      toast.error('Erro: ' + error.message);
    }
  ...
  const getStatusBadge = (status: string) => {
    if (activityLog && (status === 'running' || status === 'paused')) {
      if (activityLog.currentStatus === 'outside_hours') {
        return (
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-dark-900 via-dark-800 to-dark-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/campanhas" className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white text-lg font-bold rounded-xl transition-all duration-200 shadow-lg shadow-primary-500/40">
            <FaArrowLeft />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-dark-900 via-dark-800 to-dark-900 py-8 px-4">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div>
          <Link href="/campanhas" className="inline-flex items-center gap-3 px-6 py-3 bg-dark-700/60 hover:bg-dark-700 backdrop-blur-xl border-2 border-white/10 hover:border-primary-500/50 text-white text-base font-bold rounded-xl transition-all duration-200 mb-6">
            ...
            <div className="relative">
              <div className="flex items-start justify-between flex-wrap gap-6">
                <div className="flex-1">
                  <h1 className="text-5xl font-black text-white mb-4">{campaign.name} - API Oficial</h1>
              ...
              <div className="bg-gradient-to-br from-cyan-500/20 to-cyan-600/10 backdrop-blur-md border-2 border-cyan-500/30 rounded-2xl p-6 hover:border-cyan-500/50 transition-all">
                ...
                {nextMessageTime !== null && nextMessageTime > 0 && campaign.status === 'running' && (
                  <div className="mt-4 pt-4 border-t border-white/10">
                    ...
                    <p className="text-2xl font-black text-cyan-400 mt-2 animate-pulse">
        ...
        {activityLog && (campaign.status === 'running' || campaign.status === 'paused') && activityLog.activeAccounts && activityLog.activeAccounts.length > 0 && (
          <div className="bg-dark-800/60 backdrop-blur-xl border-2 border-white/10 rounded-2xl p-8 shadow-xl">
            <h3 className="text-3xl font-black mb-8 flex items-center gap-3">
              <span className="text-4xl">👥</span>
              Contas Ativas ({activityLog.activeAccounts.filter(a => a.isActive).length}/{activityLog.activeAccounts.length})
            </h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {activityLog.activeAccounts.map((account) => {
                ...
                return (
                  <div key={account.id} className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md border-2 border-white/20 hover:border-primary-500/50 rounded-2xl p-6 shadow-xl transition-all duration-200">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3 flex-1">
                        <span className={`text-3xl ${account.isActive ? 'text-green-400' : 'text-red-400'}`}>
                          {account.isActive ? '🟢' : '🔴'}
                        </span>
                        <div className="flex-1 min-w-0">
                          <div className="text-lg font-black truncate text-white">{account.name}</div>
                          <div className="text-sm text-white/60 truncate">{account.phone}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 mb-4">
                      <div className="bg-white/5 rounded-xl p-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`font-black text-sm ${getQualityColor(account.qualityRating)}`}>
                            {account.qualityRating === 'GREEN' && '🟢 ALTA'}
                            {account.qualityRating === 'YELLOW' && '🟡 MÉDIA'}
                            {account.qualityRating === 'RED' && '🔴 BAIXA'}
                            {account.qualityRating === 'UNKNOWN' && '⚪ N/A'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="bg-white/5 rounded-xl p-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`font-black text-sm ${getStatusColor(account.accountStatus)}`}>
                            {account.accountStatus === 'CONECTADO' && '✅'}
                            {account.accountStatus === 'ATENÇÃO' && '⚠️'}
                            {account.accountStatus === 'PROBLEMA' && '❌'}
                            {account.accountStatus === 'NÃO VERIFICADO' && '❓'}
                      ...
                      <div className="bg-white/5 rounded-xl p-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div>
                            <div className="font-black text-blue-400 text-lg">{formatNumber(account.dailyLimit)}</div>
                            <div className="text-[10px] text-white/40 font-bold">Limite</div>
                          </div>
                          <div>
                            <div className="font-black text-yellow-400 text-lg">{formatNumber(account.sentToday)}</div>
                            <div className="text-[10px] text-white/40 font-bold">Enviadas</div>
                          </div>
                          <div>
                            <div className={`font-black text-lg ${account.remaining < 100 ? 'text-red-400' : 'text-green-400'}`}>
                              {formatNumber(account.remaining)}
                            </div>
                    ...
                    {account.permanentRemoval && !account.isActive && (
