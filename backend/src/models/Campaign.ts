...
export class CampaignModel {
  ...
  static async update(id: number, campaign: Partial<Campaign>, tenantId?: number) {
    const fields: string[] = [];
    const values: any[] = [];
    let paramCount = 1;
    ...
    const result = await query(
      `UPDATE campaigns SET ${fields.join(', ')} WHERE ${whereClause} RETURNING *`,
      values
    );
    return result.rows[0];
  }
  ...
  static async updateStats(id: number, stats: {
    ...
    const result = await query(
      `UPDATE campaigns 
       ...
       RETURNING *`,
      [stats.sent_count, stats.delivered_count, stats.read_count, stats.failed_count, stats.no_whatsapp_count, stats.button_clicks_count, id, tenantId]
    );
    return result.rows[0];
  }
  ...
  static async getScheduledCampaigns(tenantId?: number) {
    // 🔒 SEGURANÇA: Se tenantId for fornecido, filtrar. Se não, retornar TODAS (para worker global)
